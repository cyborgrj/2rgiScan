...
class DataBaseUsers:
    ...
    def get_sigla_by_nome(self, nome_completo):
        self.connect_users()
        result = self.cursor.execute("SELECT user FROM users WHERE username = ?", (nome_completo,)).fetchone()
        self.cursor.close()
        self.close_users()
    ...
    def get_user_info(self, sigla):
        ...
        try:
            self.cursor.execute("SELECT username, access FROM users WHERE user = ?", (sigla.lower(),))
            found = self.cursor.fetchone()
            if found:
                nome_completo, access_hash = found

                # Verifica qual é o acesso correspondente (descriptografado)
                for acesso in ['administrador', 'digitalização', 'documentos', 'consulta']:
    ...
    def listar_nomes_usuarios(self, siglas):
        ...
        try:
            nomes = []
            for sigla in siglas:
                self.cursor.execute("SELECT username FROM users WHERE user = ?", (sigla.lower(),))
    ...
    def check_users(self, user, password):
        ...
        try:
            ...
            if found is not None:
                ...
                user_name = found[1]
                lista_acessos = ['administrador', 'digitalização', 'documentos', 'consulta', 'digitalização_simples']
...
if __name__ == "__main__":
    ...
    db_test.insert_users(user=sigla,
                            password=password,
                            access=acesso,
